...
RUNTIME_ENTRYPOINTS = {
    ...
    "provision_worker": Path(
        "/usr/local/libexec/alt-provision-worker"
    ),
    "job_stage_helper": Path(
...
class ControllerReadinessChecker:
    ...
    def static_assets_ok(self) -> bool:
        if not all(
            ...
            return False
        for key in ("bootstrap", "register_helper"):
            script = STATIC_FILES.get(key)
            if script is None or not self._run_ok(
                ["bash", "-n", str(script)],
                timeout=15,
            ):
                return False
    ...
    def systemd_checks(self) -> dict[str, bool]:
        ...
        for unit, expected in EXPECTED_UNIT_STATE.items():
            ...
            expected_load, expected_active, expected_file = (
                expected
            )
            loaded_ok = (
                loaded_ok
                and properties.get("LoadState")
                == expected_load
            )
            active_ok = (
                active_ok
    ...
    def check(self) -> dict[str, object]:
        ...
        if not result["ready"]:
            raise ControlError(
                ...
                message=(
                    "ALT deployment controller is not ready"
                ),
                exit_code=11,
                details=result,
            )
