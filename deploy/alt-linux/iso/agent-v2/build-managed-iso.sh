...
readonly execution_controller_url='https://192.168.100.17:18092'

usage() {
    printf '%s\n' \
        'Usage: build-managed-iso.sh --source <ISO> --output <ISO> --helper <BINARY> --public-key <JSON> --ca-certificate <PEM> --build-id <ID> --controller-url <URL>' >&2
    exit 2
}
...
[[ -n "$source_iso" && -n "$output_iso" && -n "$helper" &&
    -n "$public_key" && -n "$ca_certificate" && -n "$build_id" &&
    -n "$controller_url" ]] || usage
[[ "$build_id" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ ]] ||
    die 'Build ID is invalid'
[[ "$controller_url" == "$execution_controller_url" ]] ||
...
[[ -f "$ca_certificate" && -r "$ca_certificate" ]] ||
    die 'CA certificate is not readable'

root=$(cd -- "$(dirname -- "$0")" && pwd -P)
v1_iso_root=$(cd -- "$root/../agent-v1" && pwd -P)
v1_agent_root=$(cd -- "$root/../../install-agent/v1" && pwd -P)
v2_agent_root=$(cd -- "$root/../../install-agent/v2" && pwd -P)
v1_library_names=(
    config.sh dhcp-hook.sh network.sh protocol.sh state.sh transport.sh ui.sh
)
v2_library_names=(config.sh protocol.sh ui.sh)
...
input_assets=(
    ...
    "$root/initrd-overlay/lib/initrd/post/network-up/99-alt-install-execution-v2"
    "$v1_iso_root/initrd-overlay/lib/initrd/post/network-up/99-alt-install-agent-v1"
    "$v1_agent_root/alt-install-agent"
    "$v2_agent_root/alt-install-execution-agent"
)
for library_name in "${v1_library_names[@]}"; do
    input_assets+=("$v1_agent_root/lib/$library_name")
done
for library_name in "${v2_library_names[@]}"; do
    input_assets+=("$v2_agent_root/lib/$library_name")
done
input_resolved=()
public_key_resolved=
ca_certificate_resolved=
for input_index in "${!input_assets[@]}"; do
    ...
    [[ "$input_index" -ne 3 ]] || ca_certificate_resolved=$resolved
...
workdir=$(mktemp -d "$output_dir/.alt-agent-v2-build.XXXXXX")
chmod 0700 -- "$workdir"
...
cleanup() {
    case "$workdir" in
        "$output_dir"/.alt-agent-v2-build.*) rm -rf -- "$workdir" ;;
    esac
}
trap cleanup EXIT
...
ca_snapshot="$workdir/execution-ca.snapshot.pem"
install -m 0600 -- "$ca_certificate_resolved" "$ca_snapshot"
python3 - "$ca_snapshot" <<'PY' || die 'CA certificate is invalid'
import base64
import re
import ssl
import sys
...
try:
    der = base64.b64decode(
        "".join(match.group(1).splitlines()), validate=True
    )
except ValueError:
    raise SystemExit(1)
...
try:
    ...
    raise SystemExit(1)
PY
ca_sha256=$(sha256sum "$ca_snapshot" | awk '{print $1}')
[[ "$ca_sha256" =~ ^[0-9a-f]{64}$ ]] ||
    die 'CA certificate digest is invalid'
...
install -D -m 0755 "$v1_agent_root/alt-install-agent" \
    "$workdir/initrd/usr/libexec/alt-install-agent"
for library_name in "${v1_library_names[@]}"; do
    library="$v1_agent_root/lib/$library_name"
    library_mode=0644
    [[ "$library_name" != dhcp-hook.sh ]] || library_mode=0755
    install -D -m "$library_mode" "$library" \
        "$workdir/initrd/usr/libexec/alt-install-agent-lib/lib/$library_name"
done
install -D -m 0755 "$v2_agent_root/alt-install-execution-agent" \
    "$workdir/initrd/usr/libexec/alt-install-agent-v2/alt-install-execution-agent"
for library_name in "${v2_library_names[@]}"; do
    install -D -m 0644 "$v2_agent_root/lib/$library_name" \
        "$workdir/initrd/usr/libexec/alt-install-agent-v2/lib/$library_name"
done
...
install -D -m 0644 "$ca_snapshot" \
    "$workdir/initrd/usr/share/alt-install/execution-ca.pem"
printf '%s\n' "$v1_controller_url" > \
...
patch --fuzz=0 -d "$workdir/menu" -p1 < \
    "$root/boot-menu/isolinux.cfg.patch"
...
sed -i "s|__ALT_INSTALL_EXECUTION_CONTROLLER_URL__|$controller_url|g" \
    "$workdir/menu/boot/grub/grub.cfg"
grep -F '__ALT_INSTALL_EXECUTION_CONTROLLER_URL__' \
    "$workdir/menu/boot/grub/grub.cfg" >/dev/null &&
    die 'Controller URL substitution failed'

xorriso -indev "$source_iso" -outdev "$tmp_output" \
    -boot_image any replay \
    -map "$workdir/initrd-agent-v2.img" /boot/initrd.img \
...
python3 - "$source_iso" "$tmp_output" "$workdir/initrd-agent-v2.img" \
    "$workdir/initrd" "$tmp_manifest" "$build_id" "$public_key_id" \
    "$public_key_sha256" "$ca_sha256" "$controller_url" <<'PY'
import hashlib
import json
import sys
from pathlib import Path
...
build_id, public_key_id, public_key_sha256, ca_sha256, controller_url = (
    sys.argv[6:11]
)
...
document = {
    "build_id": build_id,
    "controller_ca_sha256": ca_sha256,
    "controller_url": controller_url,
    "format": "alt-install-agent-managed-iso-v2",
