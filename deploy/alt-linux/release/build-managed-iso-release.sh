...
readonly controller_url='http://192.168.100.17:18090'
readonly source_sha256='2529f98bca03a652709434a6a17cd4aac5df20c0793927abdf784e8f9388243a'

die() { printf 'alt-install-release: %s\n' "$*" >&2; exit 1; }
usage() {
    printf '%s\n' 'Usage: build-managed-iso-release.sh --source-commit <40-hex> --source-iso <ISO> --public-key <JSON> --release-id <ID> --iso-dir <DIR> --go <GO>' >&2
    exit 2
}
...
while (($#)); do
    case "$1" in
        --source-commit|--source-iso|--public-key|--release-id|--iso-dir|--go)
            (($# >= 2)) || usage
            case "$1" in
...
lock_file="$iso_dir/.alt-install-agent-v1-release.lock"
...
bash "$checkout/deploy/alt-linux/iso/agent-v1/build-managed-iso.sh" --source "$source_iso" --output "$staged_iso" --helper "$helper" --public-key "$public_snapshot" --build-id "release-$release_id" --controller-url "$controller_url"
