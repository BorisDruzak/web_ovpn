...
rollout_v2_restore() {
    ...
    systemctl stop "$rollout_v2_unit" >/dev/null 2>&1 || true
    if [[ ${ROLLOUT_V2_HAD_CURRENT:-0} == 1 ]]; then
        ln -sfn -- "$ROLLOUT_V2_OLD_CURRENT" \
            "$ROLLOUT_V2_CURRENT.new-$$" || true
        mv -Tf -- "$ROLLOUT_V2_CURRENT.new-$$" \
            "$ROLLOUT_V2_CURRENT" || true
    else
        rm -f -- "$ROLLOUT_V2_CURRENT" || true
    fi
    if [[ ${ROLLOUT_V2_HAD_UNIT:-0} == 1 ]]; then
        install -m 0644 -- "$ROLLOUT_V2_SNAPSHOT/unit" \
            "$ROLLOUT_V2_UNIT_PATH.new-$$" || true
        mv -Tf -- "$ROLLOUT_V2_UNIT_PATH.new-$$" \
            "$ROLLOUT_V2_UNIT_PATH" || true
    else
        rm -f -- "$ROLLOUT_V2_UNIT_PATH" || true
    fi
    systemctl daemon-reload >/dev/null 2>&1 || true
    if [[ ${ROLLOUT_V2_WAS_ENABLED:-0} == 1 ]]; then
        systemctl enable "$rollout_v2_unit" >/dev/null 2>&1 || true
    else
        systemctl disable "$rollout_v2_unit" >/dev/null 2>&1 || true
    fi
    if [[ ${ROLLOUT_V2_WAS_ACTIVE:-0} == 1 ]]; then
        systemctl start "$rollout_v2_unit" >/dev/null 2>&1 || true
    else
        systemctl stop "$rollout_v2_unit" >/dev/null 2>&1 || true
    fi
        ...
        case "$ROLLOUT_V2_NEW_RELEASE" in
            "$ROLLOUT_V2_RELEASES"/*)
                if [[ "$ROLLOUT_V2_NEW_RELEASE" != \
                    ...
                    rm -rf -- "$ROLLOUT_V2_NEW_RELEASE" || true
                fi
                ;;
        esac
    fi
    rm -rf -- "$ROLLOUT_V2_SNAPSHOT" || true
}
...
rollout_install_execution_v2_main() {
    ...
    [[ -n "$backup_id" && -n "$source_commit" && -n "$source_iso" &&
       ...
       -n "$pilot_record" && -n "$receipt" ]] ||
        { rollout_v2_usage; return $?; }
    for command in awk bash chmod cp curl git install ln mktemp mv python3 \
        readlink rm sha256sum systemctl; do
    ...
    local sessions backup_tool preflight_dir backup_json
    ...
    python3 "$pilot_verifier" --record "$pilot_record" \
    ...
    cp -- "$backup_json" "$ROLLOUT_V2_SNAPSHOT/backup.json"
    rm -rf -- "$preflight_dir"
...
PY
    ...
    python3 "$task6_support" verify-public-evidence \
        --evidence-dir "$task6_evidence" ||
        rollout_v2_die 'Task 6 production receipt verification failed'
    local task6_receipt="$task6_evidence/acceptance-receipt.json"
    python3 - "$task6_receipt" "$managed_iso_sha256" <<'PY'
import json, sys
value = json.load(open(sys.argv[1], encoding="utf-8"))
writes = value.get("writes", {})
before = writes.get("before_authorization", {})
after = writes.get("after_install", {})
if (value.get("schema_version") != 2 or value.get("result") != "pass" or
    value.get("acceptance_scope") != "generic-ovmf-disposable" or
    value.get("target_disk") != "/dev/vda" or
    value.get("artifacts", {}).get("iso", {}).get("sha256") != sys.argv[2] or
...
PY
    ...
    python3 - "$receipt" "$source_commit" \
        ...
        "$task6_evidence" "$pilot_record" <<'PY'
import hashlib, json, os, sys
out, commit, backup_path, sidecar_path, task6_path, evidence, pilot_path = sys.argv[1:]
backup = json.load(open(backup_path, encoding="utf-8"))
sidecar = json.load(open(sidecar_path, encoding="utf-8"))
task6 = json.load(open(task6_path, encoding="utf-8"))
