...
## Границы входных сценариев

В текущем репозитории поддерживаются только три самостоятельных Ansible
entrypoint:

- `01-preflight.yml` — регистрационный preflight;
- `02-provision-account.yml` — прежний путь создания локальной учётной записи;
- `03-configure-domain-workstation.yml` — настройка уже установленной
  рабочей станции через `workstationctl configure`.

Этот runbook использует только `03-configure-domain-workstation.yml`.
Файлы в `playbooks/tasks/` — фрагменты, подключаемые из playbook, и не должны
запускаться напрямую. Если на развёрнутом контроллере существуют другие
playbook, которых нет в текущем репозитории, они являются остатками legacy
развёртывания: не запускайте и не удаляйте их в рамках stage-03/canary без
отдельной процедуры миграции и подтверждения владельца.
