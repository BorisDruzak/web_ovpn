...
def assert_manual_mvp_contract(text: str) -> None:
    for required in (
        ...
        "The launcher validates and confirms the final hostname",
        "The operator enters only the hostname and AD login",
        "domain name, realm, workgroup and OU are controller-owned",
...
def test_manual_mvp_runbook_includes_the_pilot_acceptance_checklist() -> None:
    ...
    for required in (
        ...
        "controller automatically runs preflight, AD-user",
        "validation, preview and configuration",
