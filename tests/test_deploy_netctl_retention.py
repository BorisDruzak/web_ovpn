...
def test_systemd_verifier_accepts_captured_availability_timer_dbus_properties() -> None:
    """Linux exposes the availability schedule through DBus timer properties."""
    verifier = runpy.run_path(str(VERIFIER))
    ...
    assert verifier["property_matches"](
        "TimersCalendar", properties["TimersCalendar"], "*-*-* *:04/5:00"
    ) is True


def test_systemd_verifier_accepts_repeated_monotonic_timer_properties() -> None:
    """An obsolete OnUnitActive trigger must not be accepted with the boot trigger."""
    verifier = runpy.run_path(str(VERIFIER))
    properties = verifier["parse_show_properties"](
        ...
        "TimersCalendar={ OnCalendar=*-*-* *:04/5:00 ; next_elapse=0 }\n"
        "AccuracyUSec=30s\n"
        "Persistent=yes\n"
        "Unit=netctl-availability.service\n"
    )
