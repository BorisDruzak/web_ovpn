...
def test_connect_enables_runtime_identity_pragmas(pr_1b_database: str) -> None:
    ...
    try:
        ...
        assert [row[0] for row in conn.execute("SELECT version FROM schema_migrations ORDER BY version")] == [1, 2, 3, 4, 5, 6, 7, 8, 9]
    finally:
        conn.close()
...
def test_migration_3_is_applied_once_and_reopen_is_idempotent(
    ...
    try:
        assert first_state["versions"] == [1, 2, 3, 4, 5, 6, 7, 8, 9]
...
def test_migration_2_rollback_after_partial_copy_and_reopen_is_idempotent(
    ...
    assert first_success_state["versions"] == [1, 2, 3, 4, 5, 6, 7, 8, 9]
