...
path = Path("tests/alt_linux/test_controller_permissions.py")
text = path.read_text(encoding="utf-8")

old_imports = '''import grp
import io
import json
import os
import pwd
...
if text.count(old_imports) != 1:
    raise SystemExit("expected controller permission imports exactly once")
...
if append.strip() in text:
    raise SystemExit("permission tests already appended")
